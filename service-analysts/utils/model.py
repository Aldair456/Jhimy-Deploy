...
class FinancialStatement(Document):
    id = StringField(primary_key=True)  # Equivalente a @id @default(auto()) @db.ObjectId
    businessId = StringField(unique=True, required=True)  # Equivalente a businessId  String  @db.ObjectId @unique
    type = StringField(choices=["official", "draft"])  # Equivalente a type String?
    years = ListField(IntField())  # Equivalente a years  YearDataPoint[]
    datapoints = ListField(ReferenceField("FinancialDatapoint"))  # Relación con FinancialDatapoint
    currency = StringField(default="PEN")  # Equivalente a currency String @default("PEN")
    scaleType = StringField(default="THOUSANDS")  # Equivalente a scaleType String @default("THOUSANDS")
    status = StringField(choices=["pending", "confirmed", "cancelled", "official"])  # Equivalente a status
    createdAt = IntField()  # Equivalente a createdAt  DateTime @default(now())
    updatedAt = IntField()  # Equivalente a updatedAt  DateTime @updatedAt
